# other agent wallet Architecture Audit

**Date:** 2026-02-26
**Repo:** https://github.com/ark-labs-maintainer/agent-wallet
...
```
          ...
          +--------+---+  +-------+------+  +-----+-------+
                ...
                +----------+-----------+
                           ...
                           v
               +-----------+-----------+
               |   agent-wallet.example API        |  Cloudflare Worker
               |   (Hono + D1 + KV)   |  api.agent-wallet.example
               +-----------+-----------+
                           |
    ...
    WEB PAYMENT UI:
    +-----------------------------+
    |  pay.agent-wallet.example               |  Cloudflare Pages
...
**Ark Labs maintainer-operated.** The enclave is deployed to Evervault under Ark Labs maintainer's team UUID
(`team_e48a089e94da`). The API at `api.agent-wallet.example` is a Cloudflare Worker. Users
authenticate via Telegram 2FA and get a JWT session. Their keys live in the shared
enclave.

This is **fundamentally custodial** in the traditional sense -- Ark Labs maintainer's team controls
the enclave deployment, and key backups sit in Ark Labs maintainer's Cloudflare D1 database
...
- SwapManager detects HTLC settlement -> calls `wallet.sendBitcoin()` or internal
  ...
  extracts sighash digests -> batch-signs via agent-wallet.example API -> enclave signs with the
  private key.
...
enclave is Ark Labs maintainer-operated. To "deploy your own," you'd need an Evervault account,
...
other agent wallet's `SKILL.md` is a brilliant pattern -- a machine-readable instruction
...
The LendaSwap integration gives other agent wallet "BTC treasury, stablecoin payment" out
...
other agent wallet is **effectively custodial**: Ark Labs maintainer's team operates the enclave, stores
...
Golem has a working L402 reverse proxy with V2 binary macaroons. other agent wallet has
...
has no equivalent in other agent wallet. other agent wallet doesn't mention VTXO management at all.

### 4. No Mandatory Cloud Services
other agent wallet requires Evervault (enclave), Cloudflare Workers (API), Cloudflare D1
...
other agent wallet has MCP on the roadmap. Golem should ship an MCP server before they do.
The skills pattern in other agent wallet's `skills/` package is exactly what MCP tools
...
No claim covenant implementation exists in other agent wallet. The `vhtlc_refund_locktime`
...
segment from Golem or other agent wallet.**
...
## 14. Summary: other agent wallet vs. Golem Positioning

| Dimension | other agent wallet | Golem |
|-----------|-----------|-------|
| **Custody** | Custodial (Ark Labs maintainer-operated enclave) | Self-custodial (user-deployed) |
