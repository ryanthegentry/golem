...
- **Phase 1:** Agent uses ServerSigner (hot key encrypted on disk). Key IS present — same security model as every LN node. Acceptable and pragmatic for testnet and small mainnet amounts.
...
- **Other agent wallets on Ark** exist (custodial-enclave designs). Golem is the self-custodial / L402 / covenant-claim design point — different security model, different target.

## Current Phase

Working testnet wallet with automated refresh, dual-mode L402 gateway, and CLI. Mainnet deployment exists on Railway. Phase 1.5 covenant-receive primitives proven end-to-end on regtest. 690 passing tests.
...
### Next Priorities
1. Covenant claim daemon — production wiring of the regtest-proven receiver path (Phase 1.5 → 2 transition)
