...
**Waiting on:** Ark Labs maintainer's response to "can provisioning cover multiple cycles?" and "when will delegation orchestration ship in published SDK?"
...
**Action:** Track ts-sdk `delegate` branch. Ask Ark Labs maintainer about timeline. Consider building custom orchestration using exported primitives if SDK timeline is too slow.
...
**Action:** Waiting on Ark Labs maintainer's response. Then design the provisioning UX.
