...
- **Custodial agent wallets** (other agent wallet): Convenience + agent automation, but custodial. Trust Evervault Enclave.
...
- Hot key ServerSigner (same security model as every LN node and other agent wallet)
...
- **other agent wallet** (https://agent-wallet.example/): Ark Labs maintainer's custodial agent wallet on Ark. Same Boltz SwapManager pattern. No L402, no multisig, no covenants. Complementary — other agent wallet = custodial convenience, Golem = self-custodial with L402. Ark Labs maintainer actively helping design Golem.
