...
Ark Labs maintainer's Introspector guidance collapses Phase A and Phase B into a single step. On Ark, the covenant claim IS the primary claim path from day one. No intermediate Tier 0.5 needed.
...
2. Add the covenant claim leaf per Ark Labs maintainer's guidance: `preimage_hash + OP_INSPECTOUTPUTSCRIPTPUBKEY + introspector_tweaked_key`
...
### Round 2: Ark Labs maintainer's Introspector Guidance (March 7, 2026)

Ark Labs maintainer provided direct guidance for implementing covenants on Ark today:
...
Ryan mentioned `BoltzExchange/regtest` with `--profile ark`. Couldn't confirm this profile exists in the public repo. May be a recent addition or Ark Labs maintainer may have been referring to the Introspector's own `docker-compose.regtest.yml` which includes Boltz services. Need to check the actual compose file.
